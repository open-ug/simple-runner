...
func Reconcile(payload string, event string, runID string, logger *logger.DriverLogger) types.DriverResult {
	...
	command := []string{"flutter", "pub", "get"}

	exitCode, err := utils.ExecInContainer(ctx, cli, containerId, command, *logger)
	if err != nil || exitCode != 0 {
		return types.DriverResult{
			Success: false,
			Message: fmt.Sprintf("Error executing flutter pub get: %v", err),
		}
	}

	buildCommand := []string{"flutter", "build", "apk", "--release", "--target=/workspace/"}
	exitCode, err = utils.ExecInContainer(ctx, cli, containerId, buildCommand, *logger)
	if err != nil || exitCode != 0 {
		return types.DriverResult{
			Success: false,
			Message: fmt.Sprintf("Error executing flutter build apk: %v", err),
		}
	}
